...
def run_backtest(
    ...
    for i in range(WINDOW, n - MAX_BARS_IN_TRADE - 1, STEP):
        ...
        trend_filter = decision.get("trend", {})
        if "EXHAUSTION" in trend_filter.get("blocking_reason", ""):
            exhaustion_blocks += 1
            no_trade_count    += 1
            continue
        ...
        # CVD signal info
        tri = vol_intel.get("cvd_triangles", {})
        cvd_signal_type = tri.get("primary_signal", "NONE")
        down_seq_len    = tri.get("down_sequence", {}).get("sequence_length", 0)
        up_seq_len      = tri.get("up_sequence",   {}).get("sequence_length", 0)

        # Level type that triggered
        loc      = decision.get("location", {})
        level_type = loc.get("level_type", "NONE")
        in_lvn     = loc.get("in_lvn", False)

        # Swing profile info
        swing = vol_intel.get("swing_profiles", {})
        bull_val = swing.get("bull_val", 0.0)
        bear_vah = swing.get("bear_vah", 0.0)
        ...
        trade_log.append({
            ...
            "cvd_seq_length":    max(down_seq_len, up_seq_len),
...
def main():
    ...
    df = fetch_binance(symbol, timeframe, total_bars=3000)

...
def calculate_positions(entry_price, stop_loss, balance_usdt, symbol):
    ...
    for lev in LEVERAGE_LEVELS:
        # Margin is fixed at risk_dollars (you always risk same dollars)
        # Position value = margin * leverage
        # Qty (contracts) = position_value / entry_price
        margin         = round(risk_dollars / lev, 2)
...
def scan_strategy(strat, balance_info):
    ...
    # Funding rate check
    funding_ok = True
    skip_funding = strat.get("skip_funding", [])
    if skip_funding:
        try:
            from kucoin_executor import load_config
            import requests as _req
            _cfg = load_config()
            _fr = _req.get("https://api.bybit.com/v5/market/funding/history",
                params={"category":"linear","symbol":"BTCUSDT","limit":1},
                timeout=5).json()
            _rate = float(_fr["result"]["list"][0]["fundingRate"])
            _pct  = _rate * 100
            if _pct > 0.02:   _fs = "HIGH_POSITIVE"
            elif _pct > 0.005: _fs = "MILD_POSITIVE"
            elif _pct < -0.02: _fs = "HIGH_NEGATIVE"
            elif _pct < -0.005: _fs = "MILD_NEGATIVE"
            else: _fs = "NEUTRAL"
            if _fs in skip_funding:
                funding_ok = False
        except Exception:
            pass
    ...
    # OI state check
    oi_ok = True
    require_oi = strat.get("require_oi", None)
    oi_state   = "UNKNOWN"
    if require_oi:
        try:
            import requests as _req
            _oi = _req.get("https://api.bybit.com/v5/market/open-interest",
                params={"category":"linear","symbol":"BTCUSDT",
                        "intervalTime":"15min","limit":25},
                timeout=5).json()
            _bars = _oi["result"]["list"]
            if len(_bars) >= 2:
                _cur  = float(_bars[0]["openInterest"])
                _prev = float(_bars[-1]["openInterest"])
                _ma   = sum(float(b["openInterest"]) for b in _bars) / len(_bars)
                _chg  = (_cur - _prev) / _prev * 100 if _prev > 0 else 0
                if _cur > _ma * 1.02 and _chg > 1:
                    oi_state = "RISING_ELEVATED"
                elif _chg > 2:
                    oi_state = "RISING"
                elif _chg < -2:
                    oi_state = "FALLING"

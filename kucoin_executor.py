...
def place_stop_order(config, kucoin_symbol, side, qty, stop_price, reduce_only=True):
    ...
    body = {
        ...
        "stopPriceType": "TP",   # trigger on trade price
        "size":        int(qty),
        "reduceOnly":  reduce_only,
    }
...
def execute_trade(config, signal, state):
    ...
    # LONG:  buy limit slightly ABOVE signal price (fills on any dip to/below limit)
    # SHORT: sell limit slightly ABOVE signal price (fills immediately at or above)
    # Both use +SLIP_TOLERANCE so we always get filled near signal price
    entry_limit  = round(entry_price * (1 + SLIP_TOLERANCE), 2)

    print(f"  Placing {entry_side} limit @ ${entry_limit:,.2f}...")
    try:
        entry_order_id = place_limit_order(config, kc_symbol, entry_side, contracts, entry_limit)
    ...
    if direction == "LONG":
        tp1 = round(filled_price + stop_dist * 1.0, 2)
        tp2 = round(filled_price + stop_dist * 1.5, 2)
        tp3 = round(filled_price + stop_dist * 2.0, 2)
    else:
        tp1 = round(filled_price - stop_dist * 1.0, 2)
        tp2 = round(filled_price - stop_dist * 1.5, 2)
        tp3 = round(filled_price - stop_dist * 2.0, 2)
...
def run_monitor_loop():
    ...
    print(f"  Monitor loop started — checking every {MONITOR_INTERVAL}s")
    print(f"  Will check for signals AND monitor open positions")
    config = load_config()
    cycle  = 0
    while True:
        try:
            ...
            # Check for new signals every cycle
            check_for_new_signal(config, state)

            # Monitor existing positions every cycle
            state = load_state()  # reload after signal check may have updated state
            if state.get("active_trades"):
                monitor_positions(config, state)
            else:
                print(f"  No active trades — watching for signals")

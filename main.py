...
async def serve_dashboard():
    ...
    import os
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "dashboard_v2.html")
...
def live_latest_signal(
    ...
    # Use active strategy from automation state
    try:
        import json as _json
        _state    = _json.load(open("/root/trading/automation_state.json"))
        _strat    = _state.get("strategy", "BTC_H")
        strat_key = f"{symbol}_{_strat.split('_')[-1]}"
    except Exception:
        strat_key = f"{symbol}_H"
    ...
    try:
        from kucoin_executor import load_config, get_available_balance
        _cfg          = load_config()
        balance_usdt  = get_available_balance(_cfg)
...
def record_trade(trade: TradeRecord):
    ...
    def fut_qty(lev):
        return round((risk_usd / lev) / trade.entry_price, 6) if lev > 0 else 0

    def fut_margin(lev):
        return round(risk_usd / lev, 2) if lev > 0 else 0
...
def trade_review():
    ...
    def _clean(v):
        ...
        if isinstance(v, float) and (_np.isnan(v) or _np.isinf(v)): return None
        return v
    log_file = "trade_log.csv"
...
def handoff_trade(payload: dict):
    ...
    Fetches ATR, calculates stop/TP levels, places TP/SL on KuCoin,
    and registers the trade for trailing stop monitoring.
    """
    try:
        from kucoin_executor import (load_state, load_config, save_state,
            get_open_positions, api_post, SYMBOL_MAP)
        ...
        signal_id   = payload.get("signal_id", f"MANUAL-{int(_time.time())}")
        ...
        if not entry_price:
            return {"success": False, "error": "Entry price required"}

        # ── Get actual position from KuCoin ──────────────────────
        contracts = 1
        try:
            positions = get_open_positions(config)
            for pos in positions:
        ...
        # ── Calculate stop and TPs from signal engine ─────────────
        stop_price = payload.get("stop_price")
        tp1 = payload.get("tp1")
        tp2 = payload.get("tp2")
        tp3 = payload.get("tp3")

        # If not provided, fetch from signal engine
        if not stop_price or not tp1:
            try:
                symbol = payload.get("symbol", "BTCUSDT")
                sig_url = f"http://127.0.0.1:8000/signal/latest?symbol={symbol}&timeframe=15m&bars=120"
                import requests as _req
                sig_r = _req.get(sig_url, timeout=15)
                sig_data = sig_r.json()
                pos_data = sig_data.get("positions", {})
                atr_stop = pos_data.get("stop_price", 0)
                if atr_stop and not stop_price:
                    stop_price = atr_stop
            except Exception:
                pass

        # Fallback stop if still missing
        if not stop_price:
            stop_price = round(entry_price * (0.98 if is_long else 1.02), 2)
        ...
        # Calculate TPs
        if not tp1:
            tp1 = round(entry_price + stop_dist * 1.0 if is_long else entry_price - stop_dist * 1.0, 2)
        if not tp2:
            tp2 = round(entry_price + stop_dist * 1.5 if is_long else entry_price - stop_dist * 1.5, 2)
        if not tp3:
            tp3 = round(entry_price + stop_dist * 2.0 if is_long else entry_price - stop_dist * 2.0, 2)

        tp1 = float(tp1); tp2 = float(tp2); tp3 = float(tp3)

        # ── Place TP/SL on KuCoin via /api/v1/st-orders ──────────
        close_side = "sell" if is_long else "buy"
        tp1_qty    = max(1, round(contracts * 0.35))
        tp2_qty    = max(1, round(contracts * 0.30))
        tp3_qty    = max(1, contracts - tp1_qty - tp2_qty)
        if tp3_qty < 1: tp3_qty = 1

        placed_orders = []
        errors        = []

        for label, qty, tp_price in [("TP1", tp1_qty, tp1),
                                      ("TP2", tp2_qty, tp2),
                                      ("TP3", tp3_qty, tp3)]:
            try:
                body = {
                    ...
                    "timeInForce":        "GTC",
                    "reduceOnly":         True,
                    "triggerStopUpPrice": str(round(tp_price, 2)) if is_long else None,
                    "triggerStopDownPrice": str(round(tp_price, 2)) if not is_long else None,
                    "stopPriceType":      "TP",
                ...
                placed_orders.append({"label": label, "price": tp_price,
        ...
        trade_record = {
            ...
            "tp1":          {"price": tp1, "hit": False, "qty": tp1_qty},
            "tp2":          {"price": tp2, "hit": False, "qty": tp2_qty},
            "tp3":          {"price": tp3, "hit": False, "qty": tp3_qty},
            "leverage":     leverage,
            "status":       "OPEN",
        ...
        try:
            from telegram_notify import trade_executed
            margin = (contracts * entry_price) / leverage / 100
            trade_executed(direction, kc_symbol, entry_price, stop_price,
                          tp1, tp2, tp3, contracts, leverage, margin, config)
        except Exception:
            pass
        ...
        return {
            ...
            "errors":        errors,
            "message":       f"Trade registered — {len(placed_orders)} TP orders placed on KuCoin"
        }

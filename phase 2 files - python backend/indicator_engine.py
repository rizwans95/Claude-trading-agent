...
def compute_zigzag(
    ...
    # BOS detection — requires a confirmed bar CLOSE beyond the swing level.
    # Using high/low prints caused false positives on intrabar wicks that
    # never closed beyond the level (look-ahead on live data). Close-based
    # confirmation eliminates that leak at the cost of one bar of lag,
    # which is acceptable for a structural signal.
